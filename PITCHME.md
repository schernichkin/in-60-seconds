...
- Если не равно, уменьшаем Hp, т.к. кучу использовать не будем, в R1 указатель на info pointer Fail.
...
- Прагма INLINABLE не влияет на вероятность принятия решения о встраивании функции. 
- INLINE вынуждает компилятор использовать встраивание более агрессивно, но не гарантирует, что функция будет встроена. 
...
- Есть проверки диапазона и увеличенине указателя в каждом ридере.
...
Note:
- Синглетоны - типы имеющие ровно одно значение (и обратно: каждое значение имеет уникальный тип)
- Singletons позволяет производить любые вычисления над типами в compile-time и получать run-time значения
- Нас интересуют только натуральные числа и операция сложения. 
Определены в GHC.TypeLists, не используем напрямую, т.к. это внутренний модуль.
...
Note:
- Rebindable Syntax - так себе идея, потому что индексированные монады несовместимы c "обычными" монадами.
Но если очень хочется, то можно, а если не хочется, то и ручной биндинг - норм.
...
Note:
- Важен, потому что статический ридер эквивалентен Storable.
- Хотим убедиться, что оборачивание статического ридера в динамический - бесплатно.
- Показать результаты бенчмарка вживую.
